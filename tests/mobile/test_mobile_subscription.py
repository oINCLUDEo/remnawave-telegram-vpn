...
def _make_subscription(autopay=False):
    sub = MagicMock()
    sub.autopay_enabled = autopay
    sub.autopay_days_before = 3
...
async def test_calc_returns_price_for_known_period():
    ...
    assert result.price_kopeks == 9900 + 1100  # base + traffic, no device extra
...
async def test_calc_adds_device_surcharge():
    ...
    # 2 extra devices at 30% of 10000 each = 6000 extra
