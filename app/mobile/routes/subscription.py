...
All subscription operations are read-only from the mobile side —
purchases are initiated by creating a YooKassa payment session,
while balance deductions are the only direct mutation (via the existing
balance_kopeks field on the User model).

The mobile app never touches subscription records directly; the payment
webhook (already implemented in the bot) applies subscription changes
after a successful YooKassa payment.
"""
...
def _calc_price_kopeks(days: int, traffic_gb: int, devices: int) -> int:
    """Calculate subscription price in kopeks.

    Uses PERIOD_PRICES for the base period cost, adds traffic package price,
    and adds a per-extra-device surcharge (30% of base per extra device).
    For periods not in PERIOD_PRICES, interpolates linearly from the closest
    available period.
    """
...
async def buy_subscription(
    ...
    """Initiate a subscription purchase via YooKassa or balance deduction."""
    try:
...
async def upgrade_subscription(
    ...
    """Upgrade an existing subscription (traffic / devices / duration)."""
    valid_actions = {'traffic', 'devices', 'days'}
    if body.action not in valid_actions:
        raise HTTPException(
            status_code=400,
            detail=f'Неверный action. Допустимые: {sorted(valid_actions)}',
        )
    ...
    try:
        ...
        async with session_factory() as db:
            ...
            try:
                ...
                yk = YooKassaService()
                action_labels = {
                    'traffic': f'+{body.value} ГБ',
                    'devices': f'+{body.value} уст.',
                    'days': f'+{body.value} дн.',
                }
                payment = await yk.create_payment(
                    amount_kopeks=price_kopeks,
                ...
                logger.error('YooKassa upgrade payment failed', error=exc)
                raise HTTPException(status_code=502, detail='Ошибка создания платежа') from exc
