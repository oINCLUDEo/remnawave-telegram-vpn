...
class SubBuyRequest(BaseModel):
    ...
    return_url: str | None = Field(None, description='URL to redirect after payment')


class SubBuyResponse(BaseModel):
    """Purchase initiation response."""

    payment_url: str | None = Field(None, description='URL to open for payment')
    paid_from_balance: bool = Field(False, description='True when deducted from balance directly')
...
class SubUpgradeRequest(BaseModel):
    ...
    value: int = Field(..., ge=1, description='Amount to add (GB / count / days)')
    payment_method: str = Field('yookassa', description='Payment method: yookassa | balance')
...
class SubUpgradeResponse(BaseModel):
    ...
    price_kopeks: int = Field(0, description='Price charged in kopeks')
    price_rub: float = Field(0.0, description='Price charged in roubles')
